...
class DynamicDNSUpdater(ABC):
    ...
    and target hostname. An updater is generally responsible for updating a single name
    within a single service/protocol.
    """

    config_type_name: str = NotImplemented
    """
    This class variable defines the name to use in the program configuration to
    reference this updater. There can only be one name per updater, and it cannot
    conflict with any other updater. This is different from the instance name, which
    refers to the custom identifier given to a particular instance of an updater.
    """

    def __init__(
        self, name: str, config: ConfigTree, subscriptions: List[AddressProvider] = None
    ):
        self.name = name
        """
        The instance name of this updater. This is set by the user, and uniquely
        identifies this particular instance within the global config.
        """
        self.config = config
        """
        ...
        """
        The active subscriptions for this updater. Updaters need a source for the
        addresses they are interested in. Each updater stores a list of the providers
        from which they want to receive and possibly update addresses.
        """

    def update(self, address_update) -> None:
        """
        Ask the service to change the address of the configured name to be the address
        in the update. This method should not check if the address needs changing, that
        is done in needs_update().
        ...
        raise NotImplementedError(
            f"FIXME: Unimplemented update() in {type(self).__name__}"
        )
    ...
    def needs_update(
        ...
        answers = resolver.resolve(
            self.config["name"], address_update.rdtype, RdataClass.IN
        )
    ...
    def subscribe(self, provider: AddressProvider) -> None:
        """
        Subscribes this updater to the provider. No automatic processing becomes
        scheduled as a part of this call, it merely appends the provider to this
        updater's list of subscriptions. Other logic must then cause the updates to
        happen, by either calling needs_update() then update(), or
        update_from_subscriptions() which will handle it all.
        """
        self.subscriptions.append(provider)

    def update_from_subscriptions(self) -> None:
        """
        Asks each subscription to provide addresses, collects them all into one list,
        cleans that list such that there is only one address per address family, and
        finally submits those addresses to be updated.

        It is important to clean the list of duplicate addresses per family, because
        most updater protocols allow only a single address per record type. If your
        updater supports that, override this method and change the behavior. The cleanup
        ...
        for address in all_addresses:
            ...
            if type(address) not in cleaned_addresses:
                cleaned_addresses[type(address)] = address
            # Prefer global addresses over non-global ones.
            elif (
                (
                    isinstance(address, IPv4AddressUpdate)
                    or isinstance(address, IPv6AddressUpdate)
                )
                and address.address.is_global
                and not cleaned_addresses[type(address)].address.is_global
            ):
                cleaned_addresses[type(address)] = address
        ...
        for address in cleaned_addresses.values():
